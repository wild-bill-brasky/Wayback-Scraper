...
class NetOps():
    ...
    def __init__(self, source):
        ...
        self.source = source
        self.static_url = []
        #proxies=get_params.build_proxy_obj()
    def get_resp(self, url):
        try:
            if SourceMeta.bleeping_comp["base_url"] in url:
                url = f'{url}/'
                print(url)
            return requests.get(url, verify=True, timeout=10, headers=get_params.change_agents()).text
...
if __name__ == '__main__':
    ...
    net_ops = NetOps(SourceMeta.bleeping_comp) # Change the object passed to NetOps to change the website that is targeted
    net_ops.iterator()